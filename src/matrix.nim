...
template rewriteMulAdd*{a += `*`(s, b)}(a: var Vector, b: Vector, s: Number): Vector =
  a.mulAddInPlace(s, b)
...
proc svd*(a: Matrix): tuple[u: Matrix, sigma: DiagonalMatrix, vt: Matrix] =
  ...
  gesvd("S", "S", unsafeAddr m, unsafeAddr n, a.d, unsafeAddr m, # lda
...
proc cholesky*(a: SymmetricMatrix): LowerTriangularMatrix =
  when Number is float64:
...
proc gramMatrix*(a: Matrix): SymmetricMatrix =
  ...
  syrk(rowMajor, upper, transpose, a.cols, a.rows, 1.0, a.d, a.rows, 
    0.0, result.Matrix.d, a.cols)

proc reverseGramMatrix*(a: Matrix): SymmetricMatrix =
  result = matrix(a.rows, a.rows).SymmetricMatrix
  syrk(rowMajor, upper, noTranspose, a.rows, a.cols, 1.0, a.d, a.rows, 
...
proc gemm(a, b: Matrix, transA, transB: TransposeType): Matrix =
  ...
  result = matrix(m, n)
  gemm(rowMajor, transA, transB, m, n, k1, 1.0, a.d, a.rows, b.d, b.rows,
